...
def chk(): 
  ...
  ahmad = "-".join(uuid) 
  print("\n\n\x1b[37;1m  YOUR ID : "+ahmad) 
  try: 
    ahmad = requests.get("https://github.com/kurdup95/add.2/blob/2a4f643a642dc3f22c80ec03974deaf67f76645a/.gitignore").text 
    if ahmad in ahmad:
...
def banner():
	clear()
	print("""%s  \033[0;93m	
Author:𓆩 A G R 𓆪 Evildoer XD
Telegram:@AgREvildoer
bo kren:.        👆
tb:{nrxy tool 5$}
= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =                                                        """%(h))                                    
# VALIDASI TOKEN
...
def login_lagi334():
	try:
		...
		print('\n ba sarkawtuy daxl boo');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print('%s# akanwtaka swtawa / CP ACCOUNT '%(h))
		exit()
...
def dump_publik():
	...
	sol().print(win2)
	print(x+''+h+'•'+x+' ')
	pil = input(x+'['+p+'f'+x+'] ID FACEBOOK DANA : ')
	try:
...
def setting():
	...
	teks = '[bold yellow][01] 👽CRACK kon ID\n[02] 👽CRACK nwe ID\n[03] 👽CRACK RANDOM ID[/bold yellow]'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s👽AHMAD👽 %s/%s 👽OK:%s 👽CP:%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s👽Agr-START👽 %s/%s 👽Ok*%s 👽CP *%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
